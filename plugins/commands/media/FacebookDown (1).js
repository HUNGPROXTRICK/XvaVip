...
async function onCall({ message, args, getLang }) {
  try {
    ...
    const res = await global.GET(`https://deku-rest-api.replit.app/facebook?url=${encodeURIComponent(input)}`, {
      timeout: 120000
    });
    const data = res.data;

    if (!data.result) return message.reply(getLang("error"));

    const VideoStream = await global.getStream(data.result);
    await message.reply({
